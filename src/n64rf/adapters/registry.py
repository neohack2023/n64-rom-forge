...
ADAPTERS: dict[str, GameAdapter] = {adapter.adapter_id: adapter for adapter in (SM64_US, GOLDENEYE_007_US)}
...
def resolve_adapter(z64: bytes, adapter_id: str = "auto") -> dict[str, Any]:
    candidates = ADAPTERS.values() if adapter_id == "auto" else (get_adapter(adapter_id),)
    matches: list[dict[str, Any]] = []
    for adapter in candidates:
        match = adapter.match_canonical(z64)
